...
const BASE_SCAFFOLD = [
  "Create ONE image containing exactly {N} animation frames of the SAME character, arranged in a {COLS}x{ROWS} grid, for a {ACTION} animation.",
...
const ACTIONS = Object.freeze([
  ...
  action("snack", "idle-life", 4, { cols: 2, rows: 2 },
    [
      ...
      "chewing with a content closed-eye smile, one tiny crumb near the mouth",
  ...
  action("wander", "idle-life", 4, { cols: 2, rows: 2 },
    [
      "mid-step walking pose, left foot forward, looking ahead curiously",
      "mid-step walking pose, right foot forward",
      "mid-step walking pose, left foot forward again (cycle)",
  ...
  action("error-comfort", "context", 4, { cols: 2, rows: 2 },
    [
      "worried face, eyebrows angled up, a sweat drop, hands clasped nervously",
  ...
  action("break-reminder", "context", 2, { cols: 2, rows: 1 },
    [
      "holding up a small blank white sign with both hands, a caring concerned face",
      "the same pose with the sign tilted slightly, a small clock floating near the top corner",
    ],
    { totalMs: 5000, loop: "pingpong", overlayText: "break" },
    { type: "breakReminder", continuousWorkMs: 3600000, cooldownMs: 3600000 }),

  action("celebration", "context", 3, { cols: 3, rows: 1 },
    [
      "crouching slightly, about to jump, excited",
      "jumping up, both arms raised, eyes shut with joy, feet off the ground",
      "landing happily, arms wide, a few confetti specks touching the body",
    ],
    { totalMs: 3500, loop: "once" },
    { type: "tokenMilestone", stepTokens: 100000, cooldownMs: 0 }),

  // ── touch ─────────────────────────────────────────────────────────────
  action("dizzy", "touch", 3, { cols: 3, rows: 1 },
    [
      "dazed, spiral eyes, one hand on its head, tilted to the left",
      "wobbling upright, spiral eyes, a small star touching the head",
